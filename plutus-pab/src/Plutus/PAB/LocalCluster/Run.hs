...
runWith userContractHandler = do
  withLocalClusterSetup $ \dir lo@LogOutputs{loCluster} ->
...
setupPABServices :: forall a.
    ...
    => BuiltinHandler a
    -> String -> Int -> FilePath -> RunningNode -> IO ()
setupPABServices userContractHandler walletHost walletPort dir rn = do
  void $ async $ do -- TODO: better types for arguments
